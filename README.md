...
1. **自动签到**：每天在北京时间 08:00-23:00 之间随机时间签到，无需人工操作。
...
定时任务由 Cloudflare Cron 驱动，默认每 30 分钟唤醒一次 Worker。Worker 会在 D1 中为每天生成一个北京时间 08:00-23:00 之间的随机目标时间；如果当前时间已经超过目标时间且当天还没执行，就立即签到并记录当天已执行。
...
默认是随机签到：Cloudflare 每 30 分钟触发一次 Worker，Worker 在北京时间 08:00-23:00 之间随机选择当天目标时间。修改 Cron 频率的方法：

1. 编辑仓库根目录的 `wrangler.jsonc` 中 `triggers.crons` 字段。
2. Cron 表达式格式为 `分 时 日 月 周`，例如 `*/15 * * * *` 表示每 15 分钟唤醒一次，随机执行延迟会更小。
